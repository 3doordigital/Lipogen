	...
	if ( ! class_exists( 'WC_Smart_Coupons' ) ) {
		...
		class WC_Smart_Coupons {
			...
			public function __construct() {
				...
				if ( Smart_Coupons_WC_Compatibility::is_wc_22() || Smart_Coupons_WC_Compatibility::is_wc_21() ) {
				...
				if( isset( $_GET['import'] ) && $_GET['import'] == "woocommerce_coupon_csv" ||
					isset( $_GET['page']) && $_GET['page'] == 'woocommerce_coupon_csv_import' ) {
					ob_start();
				}
			 ...
			 */
			public function generated_coupon_details_after_order_table( $order = false ) {
				if ( !empty( $order->id ) ) {
			...
			public function apply_coupon_from_url() {
				...
				if ( isset( $coupon_args['coupon-code'] ) && ! empty( $coupon_args['coupon-code'] ) ) {
					...
					if ( Smart_Coupons_WC_Compatibility::global_wc()->cart->has_discount( $coupon_args['coupon-code'] ) ) {
						return;
					}
					...
					} elseif ( ! empty( Smart_Coupons_WC_Compatibility::global_wc()->cart ) ) {
			...
			public function save_called_credit_details_in_order( $order_id, $posted ) {
				...
				$order = Smart_Coupons_WC_Compatibility::get_order( $order_id );
			...
			/**
			 * Save entered credit value by customer in order itme meta
			 * 
			 * @param int $item_id 
			...
			public function save_called_credit_in_session( $cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data ) {
				...
				$_product = Smart_Coupons_WC_Compatibility::get_product( $product_id );
			...
			public function smart_coupons_json_search( $x = '', $post_types = array( 'shop_coupon' ) ) {
				...
				$all_discount_types = ( Smart_Coupons_WC_Compatibility::is_wc_22() || Smart_Coupons_WC_Compatibility::is_wc_21() ) ? wc_get_coupon_types() : $woocommerce->get_coupon_discount_types();
			 ...
			 */
			public function show_store_credit_balance( $order, $send_to_admin ) {
					...
					$js = " jQuery(function(){
								...
								jQuery('div.apply_coupons_credits').on('click', function() {
									...
									if( coupon_code != '' && coupon_code != undefined ) {
										...
										var url = '". home_url() ."?sc-page=". $page ."&coupon-code='+coupon_code;   
			...
			public function gift_certificate_receiver_detail_form() {
				...
				foreach ( Smart_Coupons_WC_Compatibility::global_wc()->cart->cart_contents as $product ) {
					...
					$_product = Smart_Coupons_WC_Compatibility::get_product( $product['product_id'] );
			...
			public function show_attached_gift_certificates() {
				...
				$_product = Smart_Coupons_WC_Compatibility::get_product( $post->ID );
				...
				if ($coupon_titles) {
					...
					<script type="text/javascript">
						...
						var updateSendCouponOnRenewals = function() {
							...
							<?php if( Smart_Coupons_WC_Compatibility::is_wc_22() || Smart_Coupons_WC_Compatibility::is_wc_21() ) { ?>
			...
			public function woocommerce_gift_certificates_email_template_path( $template ) {
				...
				$template = locate_template(
					array(
						substr( plugin_basename( __FILE__ ), 0, strpos( plugin_basename( __FILE__ ), '/' ) + 1 ) . $template_name,
						$template_name
					)
				);
						...
						<div class="panel woocommerce_options_panel">
							<div class="option_group">
								...
								<p class="form-field">
									...
									<?php
										$coupon_types = ( Smart_Coupons_WC_Compatibility::is_wc_22() || Smart_Coupons_WC_Compatibility::is_wc_21() ) ? wc_get_coupon_types() : $woocommerce->get_coupon_discount_types();
								...
								<p class="form-field">
									...
									<img class="help_tip" data-tip='<?php _e( 'Value of the coupon.', 'wc_smart_coupons' ) ?>' src="<?php echo Smart_Coupons_WC_Compatibility::global_wc()->plugin_url(); ?>/assets/images/help.png" height="16" width="16" /></p>
								</p>
								...
								<p class="form-field">
									...
									<input type="text" name="sc_expiry_date" id="sc_expiry_date" placeholder="<?php _e( 'YYYY-MM-DD', 'wc_smart_coupons' );?>" />                                                
								</p>
								...
								<p class="form-field">
									...
									<img class="help_tip" data-tip='<?php _e( 'This field allows you to set the minimum subtotal needed to use the coupon.', 'wc_smart_coupons' ) ?>' src="<?php echo Smart_Coupons_WC_Compatibility::global_wc()->plugin_url(); ?>/assets/images/help.png" height="16" width="16" /></p>
								</p>
										...
										if( isset( $post ) ) {
											...
											if ( $product_ids ) {
												...
												foreach ( $product_ids as $product_id ) {
													...
													$product = Smart_Coupons_WC_Compatibility::get_product( $product_id );

													$product_name = Smart_Coupons_WC_Compatibility::get_formatted_product_name( $product );
								...
								<p class="form-field">
									...
									<input type="text" name="sc_customer_emails" id="sc_customer_emails" class="input-text" placeholder="<?php _e( "No restrictions", "wc_smart_coupons" ); ?>"/>
								...
								<p class="form-field">
									...
									<img class="help_tip" data-tip='<?php _e( 'How many times this coupon can be used before it is void.', 'wc_smart_coupons' ) ?>' src="<?php echo Smart_Coupons_WC_Compatibility::global_wc()->plugin_url(); ?>/assets/images/help.png" height="16" width="16" /></p>
								</p>
								...
								<p class="form-field">
									...
									<img class="help_tip" data-tip='<?php _e( 'The maximum number of individual items this coupon can apply to when using product discounts. Leave blank to apply to all qualifying items in cart.', 'wc_smart_coupons' ) ?>' src="<?php echo Smart_Coupons_WC_Compatibility::global_wc()->plugin_url(); ?>/assets/images/help.png" height="16" width="16" /></p>
								</p>
								...
								<p class="form-field">
									...
									<img class="help_tip" data-tip='<?php _e( 'How many times this coupon can be used by an invidual user. Uses billing email for guests, and user ID for logged in users.', 'wc_smart_coupons' ) ?>' src="<?php echo Smart_Coupons_WC_Compatibility::global_wc()->plugin_url(); ?>/assets/images/help.png" height="16" width="16" /></p>
								</p>  
